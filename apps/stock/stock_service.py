...
class StockService:
        ...
        try:
            ...
            for s in shorts:
                ...
                if stock:
                    ...
                    gross_profit = float(s.total_proceeds) - float(current_value)
    ...
    def sell_short(user_id: str, symbol_code: str, quantity: int) -> Tuple[bool, str, Optional[Dict]]:
        ...
        db = SessionLocal()
        try:
            # 1. Check user blacklist status
            from apps.collections.collections_service import is_blacklisted
            if is_blacklisted(user_id):
                return False, "信用取引制限のため、空売りは実行できません。", None
            ...
            if not stock.is_tradable:
                return False, "この銘柄は現在取引停止中です", None

            # 2. Check stock account
            account = db.query(StockAccount).filter_by(user_id=user_id, is_active=True).first()
            if not account:
                return False, "株式口座が開設されていません", None

            # 3. Calculate Margin Requirement (50%)
            price = stock.current_price
            trade_value = price * quantity
            margin_required = Decimal(str(trade_value)) * Decimal('0.5')

            # 4. Check & Transfer Margin from User Bank to Reserve
            bank_account = banking_api.get_account(account.linked_bank_account_id)
            if not bank_account:
                return False, "連携銀行口座が見つかりません", None
            
            if float(bank_account['balance']) < float(margin_required):
                return False, f"委託証拠金不足です（必要額: ¥{margin_required:,.0f}）", None

            # Transfer Margin
            try:
                tx = banking_api.transfer(
                    from_account_number=bank_account['account_number'],
                    to_account_number=RESERVE_ACCOUNT_NUMBER,
                    amount=float(margin_required),
                    currency='JPY',
                    description=f"信用取引証拠金 ({stock.symbol_code})"
                )
            except Exception as bank_err:
                print(f"[StockService] margin transfer failed: {bank_err}")
                return False, "証拠金の振替に失敗しました", None

            # 5. Create Short Position
            # 返済期日デフォルト: 6ヶ月後
            due_date = now_jst() + timedelta(days=180)
            
            new_position = UserStockShortPosition(
                user_id=user_id,
                symbol_id=stock.symbol_id,
                quantity=quantity,
                average_sell_price=Decimal(str(price)),
                total_proceeds=Decimal(str(trade_value)),
                stock_account_id=account.stock_account_id,
                due_date=due_date
            )
            db.add(new_position)

            # 6. Update Account Margin Deposit & Stock Short Interest
            account.margin_deposit += margin_required
            ...
            db.rollback()
            print(f"空売りエラー: {e}")
            return False, f"空売り処理中にエラーが発生しました: {str(e)}", None
        finally:
            db.close()
        ...
        try:
            ...
            if transfer_amount > 0:
                banking_api.transfer(
                    ...
                    currency='JPY',
                    description=f"信用返済益/証拠金返還 ({stock.symbol_code})"
                )
            elif transfer_amount < 0:
                shortfall = abs(transfer_amount)
                if bank_account.balance < shortfall:
                    return False, "買い戻しによる損失を補填する残高が不足しています", None
                banking_api.transfer(
                    from_account_number=bank_account.account_number,
                    to_account_number=RESERVE_ACCOUNT_NUMBER,
                    amount=float(shortfall),
                    currency='JPY',
                    description=f"信用返済損充当 ({stock.symbol_code})"
                )
            ...
            transaction = StockTransaction(
                ...
                quantity=quantity,
                price=Decimal(str(stock.current_price)),
                total_amount=cost_to_cover, # 買い戻しコストを記録
                stock_account_id=stock_account.stock_account_id
            )
            db.add(transaction)
            ...
            return True, "買い戻しが完了しました", {
                ...
                'margin_returned': float(total_margin_released),
                'transaction_id': transaction.transaction_id
            }
